...
public class search_dmap0005_ST extends search
{
    String rtnVal = "";
    ...
    public void search(RestHighLevelClient client , SearchSourceBuilder ssb, int page , int size ,String[] excludeFields,
                    String docId, String openDataYn)
    {
        try
        {
            //전처리
            ...
            if (openDataYn.equals("Y"))
                request.setRequest(new SearchRequest("dmap_map_tot_open_list_meta_info"));//index
            else 
                request.setRequest(new SearchRequest("dmap_map_tot_org_open_list_meta_info"));//index
                

            request.setScriptType(ScriptType.STORED);
            request.setScript("dmap_0005");//script title
